...
```{r model-2-1, cache=TRUE, results='asis'}
...
```
 ...
 How much of the work is that one predictor doing on its own? Fitting it alone puts the comparison on a fair footing: one column about the publisher against eight columns about the event.

```{r model-2-2, cache=TRUE, results='asis'}
# Fit the new model
...
Adding more variables to the calculations will multiply the columns: e.g. 20 event types with 40 actor roles will result in 800 columns. Fitting the sample data into this will take minutes for this limited data set. So, to take a shortcut, we'll lump actors together thus reducing the number of columns. 
...
```{r tone-interaction-prep1, cache=TRUE, results='asis'}
...
The levels to keep are chosen from the fitting data only, then applied unchanged to the test data. This is the stricter of the two approaches used in this report: the earlier lump_top() picks its levels from the whole of model_data, before the split. Neither choice looks at the response, so neither leaks the answer - but selecting levels on the training rows alone is the safer habit, and costs nothing here.


```{r tone-interaction-prep2, cache=TRUE, results='asis'}
...
The comparison has to be just. So the additive model is refitted on exactly the same rows and the same collapsed factors. Only then is the nested test meaningful.
...
Let's visualize how, an interaction would look like if it mattered: lines that cross rather than # lines that shift up and down together.

```{r tone-interaction-plot, cache=TRUE, fig.width=6.5, fig.height=4.2, fig.align='center', fig.cap="Mean tone by event type.", out.width='13.0 cm', out.height='7 cm'}
...
## Tone results
Let's summarize what, we've seen regarding the tone: 

```{r tone-results1, cache=TRUE, results='asis'}
...
tone_results <- data.frame(
  ...
  # Add addition colums for the Improvements and RMSE
  mutate(
...
Let's compare the models and get improvements if any:

```{r tone-results2, cache=TRUE, results='asis'}
...
Selecting the best tone model, let's visualize the predicted tone against the actual tone.

```{r tone-predicted-vs-actual, cache=TRUE, fig.width=6.5, fig.height=3.5, fig.align='center', fig.cap="Predicted vs actual tone on a random sample of 5,000 test rows.", out.width='14.0 cm', out.height='6.8 cm'}
...
# Conclusion
Looking back on this project, let's take a look at the results:
...
The second question, added once the first had run its course, was whether those same characteristics predict *how* the coverage reads rather than how much of it there is. That answer is different: **substantially better, though still far from precise**. It does not seem, that more characteristics add any more precision in how, the tone changes. 
...
* **Ungeolocated events are the best covered.** Events GDELT cannot place on a map draw roughly two and a half times the median coverage of ones it can - so the lack of location is informative, and dropping those rows would discard the most-covered events.
...
2. **The response is quantized.** `NumArticles` is heaped on small round integers, with even values strongly favored. Group medians can only land on a handful of distinct values, which puts a floor under how small a difference is even detectable.
...
* **Change the question to classification.** Instead of predicting the count, predict whether an event finishes in the top 5% of coverage - did it go viral? A rare-event classification problem has cleaner and more interpretative metrics than an R-squared of a few percent, and "which events break out" is arguably the more interesting question anyway.
