...
required_packages <- c("here", "readr", "dplyr", "tidyr", "lubridate", "purrr",
                       "ggplot2", "car", "lmtest", "tinytex", "countrycode", "kableExtra",
                       "scales", "rpart", "MASS")
...
```
It seems, that the 
Let's check to see how the model behaves. Does the model's errors vary, or are they the same across the board? We can use the [Breusch–Pagan test](https://en.wikipedia.org/wiki/Breusch%E2%80%93Pagan_test). 

```{r model-1-tests_2, cache=TRUE, results='asis'}
# Calculate the Breusch–Pagan test data
bp <- lmtest::bptest(model_1)
cat("Breusch-Pagan test: BP = ", round(unname(bp$statistic), 1),
    ", df = ", unname(bp$parameter),
    ", p = ", format.pval(bp$p.value, digits = 3), "\n", sep = "")
```

The residual plot shows the striping that the quantisized response makes inevitable - with the raw counts heaped on small round integers, the residuals can only fall on a set of discrete tracks rather than forming an even cloud. The Breusch-Pagan test rejects constant variance, which at this sample size it would do for almost any departure however small, so it is reported for completeness rather than treated as decisive. The variance inflation factors are the more useful check, and they are unremarkable: the predictors are not standing in for one another.

There is one further assumption that no test in `lmtest` will catch, and it matters more than either of the above: **independence of observations**. It is violated by construction, because most rows share an article with other rows. The consequence is that the standard errors printed by `summary()` are too small - the model has far fewer independent observations than it has rows. It does not bias the coefficients, and it does not affect the held-out RMSE figures used to compare models, which is why those are what this chapter relies on.

## Model #2 - refined model

The exploration chapter found that source domain separated coverage better than any attribute of the event itself. Model #2 tests that directly by adding it to Model #1 and changing nothing else.
...
That is worth being precise about. It is a statement about how GDELT samples the news: some outlets are systematically associated with higher article counts than others. It is not a statement about journalism, about editorial judgement, or about bias, and nothing in this data would support such a claim.
...
If the tree does substantially better than Model #2, the linear form was the limitation. If it does not, the limitation is the data.

```{r model-3, cache=TRUE, results='asis'}

# A regression tree on exactly the same predictors as Model #2. cp is set low and
# depth capped so the tree is allowed to grow well past what is likely useful -
# the point is to give the flexible model every chance.
...
```{r results-plot, cache=TRUE, fig.width=6.5, fig.height=3.5, fig.align='center', fig.cap="Predicted against actual coverage on the held-out test set for the best model, on a random sample of 5,000 test rows.", out.width='14.0 cm', out.height='6.8 cm'}
...
# Negative binomial rather than Poisson: the counts are heavily overdispersed, which
# a Poisson model assumes away. Fitted on a smaller sample - glm.nb estimates the
# dispersion parameter iteratively and is far slower than lm().
set.seed(4)
nb_fit_rows <- sample(seq_len(nrow(fit_set)), min(30000, nrow(fit_set)))
nb_fit <- fit_set[nb_fit_rows, ]

# expm1() undoes the log1p() used to build the response, recovering the raw count
...
```{r tone-distribution, cache=TRUE, fig.width=6.5, fig.height=3.5, fig.align='center', fig.cap="Distribution of AvgTone across all events in the month.", out.width='14.0 cm', out.height='6.8 cm'}
...
```{r tone-by-eventtype, cache=TRUE, fig.width=6.5, fig.height=4.6, fig.align='center', fig.cap="Mean AvgTone by CAMEO event root code, ordered from most negative to most positive.", out.width='13.0 cm', out.height='7 cm'}
