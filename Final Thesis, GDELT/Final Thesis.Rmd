...
```{r model-1-tests, cache=TRUE, results='asis'}
...
#
# The column taken below, GVIF^(1/(2*Df)), is rescaled so terms with different degrees
...
# Heteroskedasticity: constant residual variance is one of the assumptions behind
# the standard errors of a linear model.
...
The residual plot shows the striping that the quantized response makes inevitable - with the raw counts heaped on small round integers, the residuals can only fall on a set of discrete tracks rather than forming an even cloud. The Breusch-Pagan test rejects constant variance, which at this sample size it would do for almost any departure however small, so it is reported for completeness rather than treated as decisive. The variance inflation factors are the more useful check, and they are unremarkable: the predictors are not standing in for one another.
