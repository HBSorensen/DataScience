...
kable(tools_tab, row.names = FALSE, booktabs = TRUE, linesep = "\\hline")
```
...
Now, that we have the data in the geomag_data, sunspots_data, ugs_data and goes_data tables, we can start taking a look at the data in more details. Let's start visualizing the data on a time line. First, we'll start by setting up a few conditions: 

1. Due to space constraints, I'll will only show data based on the month, not the daily/hourly/minute/second information. 
2. We'll set up a function for plotting the data, so we don't need to repeat code.

```{r data-display-function, cache=TRUE, results='asis'}

# Earthquakes: average magnitude per month
eq_monthly <- ugs_data %>%
  mutate(month = floor_date(time, "month")) %>%
  group_by(month) %>%
  summarise(avg_mag = mean(mag, na.rm = TRUE), .groups = "drop")

# Geomagnetic Ap index: monthly mean
geomag_monthly <- geomag_data %>%
  mutate(month = floor_date(make_date(year, month, day), "month")) %>%
  group_by(month) %>%
  summarise(avg_Ap = mean(Ap, na.rm = TRUE), .groups = "drop")

# Sunspot number: monthly mean (-1 marks a missing daily reading)
sunspot_monthly <- sunspots_data %>%
  mutate(total = na_if(total, -1)) %>%
  mutate(month = floor_date(make_date(year, month, day), "month")) %>%
  group_by(month) %>%
  summarise(avg_sunspots = mean(total, na.rm = TRUE), .groups = "drop")

# GOES X-ray flux (xl channel): monthly mean
goes_monthly <- goes_data %>%
  mutate(month = floor_date(time, "month")) %>%
  group_by(month) %>%
  summarise(avg_xl = mean(xl, na.rm = TRUE), .groups = "drop")

# Create a generic function for displaying earthquakes in relation to other data
plot_dual_axis_earthquakes <- function(secondary, secondary_col, 
                                       secondary_label,
                                       title) {

  # Rescale the secondary series into the earthquake series' range so 
  # both draw together
  primary_range   <- range(eq_monthly$avg_mag, na.rm = TRUE)
  secondary_range <- range(secondary[[secondary_col]], na.rm = TRUE)
  scale_factor <- diff(primary_range) / diff(secondary_range)
  offset <- primary_range[1] - secondary_range[1] * scale_factor

  secondary <- secondary %>%
    mutate(scaled = .data[[secondary_col]] * scale_factor + offset)

  # Only show data inside the 1975-2026 range
  plot_start <- as.Date("1975-01-01")
  plot_end   <- as.Date("2026-12-31")
  eq_plot_data <- eq_monthly %>% filter(month >= plot_start & month <= plot_end)
  secondary    <- secondary %>% filter(month >= plot_start & month <= plot_end)

  # Plot the actual data
  ggplot() +
    geom_line(data = eq_plot_data, aes(x = month, y = avg_mag, color = "Avg. earthquake magnitude"),
              linewidth = 0.7) +
    geom_line(data = secondary, aes(x = month, y = scaled, color = secondary_label),
              linewidth = 0.7) +
    # Map the two series to explicit colors so a legend box is drawn for each
    scale_color_manual(
      name = NULL,
      values = setNames(c("steelblue", "firebrick"), c("Avg. earthquake magnitude", secondary_label))
    ) +
    scale_y_continuous(
      name = "Avg. earthquake magnitude",
      sec.axis = sec_axis(~ (. - offset) / scale_factor, name = secondary_label)
    ) +
    scale_x_date(date_breaks = "1 year", date_labels = "%Y",
                 limits = c(plot_start, plot_end)) +
    # Zoom the view to start at magnitude 3 (we don't record anything below that)
    # without dropping any underlying data
    ...
    theme(
      ...
      panel.grid.minor.x = element_blank(),  # no sub-year gridlines
      legend.position = "bottom"
    )
}
```

### Earthquakes vs geomagnetism
```{r data-display-earthquakes-geomag, cache=TRUE, results='asis'}
# Graph 1: Earthquakes vs Geomagnetic Ap index
plot_dual_axis_earthquakes(geomag_monthly, "avg_Ap", "Avg. Ap index",
               "Earthquakes vs Geomagnetic Ap Index")

# Graph 2: Earthquakes vs Sunspot number
plot_dual_axis_earthquakes(sunspot_monthly, "avg_sunspots", "Avg. sunspot number",
               "Earthquakes vs Sunspot Number")

# Graph 3: Earthquakes vs GOES X-ray flux
plot_dual_axis_earthquakes(goes_monthly, "avg_xl", "Avg. X-ray flux (xl, W/m²)",
               "Earthquakes vs GOES X-ray Flux")
